...
def Noise_Data(backend, shots, qubits, simulator):
    ...
    pa = 1 - exp(-(backend_data['qubits'][Battery][5]['value']*0.001 )/ backend_data['qubits'][Battery][0]['value'])  # T1 relaxation probability
    pd = 1 - exp(-(backend_data['qubits'][Battery][5]['value']*0.001)/ backend_data['qubits'][Battery][1]['value'])     # T2 dephasing probability
